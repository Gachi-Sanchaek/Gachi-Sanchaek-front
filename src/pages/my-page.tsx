...
export default function MyPage() {
  ...
  const { data: bonggongs, isLoading: bonggongsLoading, error: bonggongsError } = useBonggongs();
  ...
  const handleNicknameChange = (value: string) => {
    const filteredValue = value.replace(/[^a-zA-Z0-9\u3131-\u318E\uAC00-\uD7A3]/g, '');
  ...
  const sortedPointHistory = [...pointLogs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  if (bonggongsLoading || pointsLoading) return <div className='w-full h-screen flex justify-center items-center font-[PretendardVariable] text-[#FFFFFF]'>로딩중...</div>;
  if (bonggongsError || pointsError || profileError) return <div className='w-full h-screen flex justify-center items-center font-[PretendardVariable] text-[#FFFFFF]'>{(bonggongsError || pointsError || profileError) as string}</div>;
  if (!profile) return <div className='w-full h-screen flex justify-center items-center font-[PretendardVariable] text-[#FFFFFF]'>사용자 정보가 없습니다.</div>;
        ...
        <div className='p-5 pt-2'>
          <div className='flex items-center mt-4'>
            <div className='relative w-[50px] h-[50px] bg-[#FFFFFF] rounded-full flex-shrink-0 flex items-center justify-center overflow-visible shadow-[0_0_8px_0_rgba(0,0,0,0.3)]'>
              <img src={`${import.meta.env.VITE_API_URL}${profile?.profileImageUrl}`} alt='프로필 봉공' className='w-[50px] h-[50px] rounded-full' />
              <button onClick={handleEditClick} className='absolute bottom-0 right-0 w-4 h-4 flex items-center justify-center'>
                <img src={pen} alt='수정펜' className={isEditing ? 'grayscale' : 'opacity-100'} />
              </button>
            </div>
            <div className='ml-2 flex items-center'>
              {isEditing ? (
                ...
                <h2 className='text-white font-[PretendardVariable] font-medium text-[22px]'>{profile?.nickname}님</h2>
              )}
            </div>
          </div>
          ...
          <div className='mt-4 flex items-center text-white font-medium'>
            <div className='flex items-center rounded-lg bg-black/10 p-2'>
              ...
              <span className='ml-2 font-[PretendardVariable] text-base'>{profile?.walkingCount}번</span>
            </div>
            <span className='ml-1 font-[PretendardVariable] text-base'>의 산책을 하고</span>
          </div>

          <div className='mt-1.5 flex items-center text-white font-medium'>
            <div className='flex items-center rounded-lg bg-black/10 p-2'>
              <img src={coin} alt='동전 아이콘' className='w-4.5 h-4.5' />
              <span className='ml-2 font-[PretendardVariable] text-base'>{profile?.totalPoints.toLocaleString()}포인트</span>
            </div>
            <span className='ml-1 font-[PretendardVariable] text-base'>를 적립했어요!</span>
          </div>
          ...
          <div className='absolute bottom-[51vh] left-1/2 transform -translate-x-1/2 flex gap-14'>
            ...
            >
              <span className={`text-base font-[PretendardVariable] ${activeTab === 'points' ? 'text-white font-semibold' : 'text-white/70'}`}>포인트</span>
              {activeTab === 'points' && <img src={polygon} alt='삼각형 아이콘' className='w-6 h-6 mt-0.5' />}
            </div>

            <div className='flex flex-col items-center cursor-pointer' onClick={() => setActiveTab('stamps')}>
              <span className={`text-base font-[PretendardVariable] ${activeTab === 'stamps' ? 'text-white font-semibold' : 'text-white/70'}`}>스탬프</span>
              {activeTab === 'stamps' && <img src={polygon} alt='삼각형 아이콘' className='w-6 h-6 mt-0.5' />}
            </div>
          </div>
        </div>
      }
          ...
          {activeTab === 'points' ? (
            <div className='relative h-full'>
              <div className='sticky top-0 left-0 z-10 pt-2 pb-3 flex items-center'>
                <div className='flex items-center gap-1 text-sm text-[#BDBDBD] cursor-pointer select-none' onClick={() => setDropdownOpen(!dropdownOpen)}>
                  <span className='font-[PretendardVariable] font-light text-[16px] text-[#BDBDBD] cursor-pointer flex items-center gap-1'>{selectedMonth}</span>
                  {dropdownOpen ? <ChevronUp size={20} className='text-[#BDBDBD]' /> : <ChevronDown size={20} className='text-[#BDBDBD]' />}
                </div>
                {dropdownOpen && (
              ...
              <div className='overflow-y-auto h-[calc(100%-40px)] pb-5'>
                {filteredPointHistory.length === 0 ? (
                  <div className='w-full h-full flex flex-col justify-center items-center text-center text-[#BDBDBD] font-[PretendardVariable]'>
                    ...
                    <p className='text-[14px] mt-1'>산책하고 포인트를 모아보세요!</p>
                  </div>
                ) : (
                  <div className='flex flex-col gap-3'>
                    {filteredPointHistory.map((p) => {
                      ...
                      return (
                        <div key={p.date} className='flex justify-between h-16 items-center font-[PretendardVariable]'>
                          <span className='text-[#BDBDBD] font-semibold text-[16px] w-15'>{formattedDate}</span>
                          <div className='flex flex-col flex-1 ml-2 justify-center'>
                            <span className='font-medium text-[16px]'>{walkAuthType(p.type)}</span>
                            <span className='text-[#BDBDBD] text-[14px]'>{p.location.length > 15 ? `${p.location.slice(0, 15)}...` : p.location}</span>
                          </div>
                          <span className='text-[#5FD59B] text-[16px] font-semibold'>{p.amount.toLocaleString()}P</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className='grid grid-cols-3 gap-3 p-2'>
                {bonggongs?.data.map((b) => {
                  ...
                  return (
                    ...
                    >
                      <img src={`${import.meta.env.VITE_API_URL}${b.imageUrl}`} alt={b.name} className={`w-28 h-28 object-contain rounded-lg ${b.isActive ? '' : 'filter grayscale'}`} />
                      {isSelected && (
                      ...
                      {!b.isActive && (
                        <div className='absolute bottom-1 right-1 w-6 h-6 bg-[#BDBDBD] rounded-full flex items-center justify-center'>
                          <img src={lock} alt='잠금 아이콘' className='w-3 h-3' />
                        </div>
                      )}
                    </div>
                  );
