...
const Home = () => {
  ...
  const { profile, ranking, error, fetchProfile, fetchRanking } = useUserStore();
      ...
      greenChildren={
        <div className='p-5 pt-2'>
          <div className='flex items-center mt-4'>
            ...
            <h2 className='ml-2 font-[PretendardVariable] text-[24px] text-[#FFFFFF]'>{profile.nickname}님</h2>
          ...
          <div className='absolute left-0 bottom-[60vh] flex justify-center w-full px-4 mt-25 overflow-visible'>
            ...
            <div className='absolute top-full w-full flex justify-between text-sm mt-2 px-4'>
              <span></span>
              <span className='text-right text-[#FFFFFF]'>{nextPoints.toLocaleString()}P</span>
            </div>
            ...
            >
              <img src={walkBonggong} alt='산책 봉공' className='w-[80px] h-[80px]' />

              <div className='absolute top-[-10px] left-1/2 transform -translate-x-1/2 text-xs px-2 py-0.5 rounded-md font-medium bg-[#FFFFFF] text-[#5FD59B]'>{currentPoints.toLocaleString()}</div>
            </div>
          </div>
        </div>
      }
      whiteChildren={
        <div className='flex flex-col h-full gap-6'>
          ...
          <div className='flex-[3] flex gap-4'>
            <div className='bg-[#FFFFFF] flex-1 flex flex-col justify-center rounded-xl p-5 shadow-[0_0_10px_0_rgba(0,0,0,0.08)] mx-auto cursor-pointer' onClick={() => navigate('/mypage')}>
              <div className='flex items-center'>
                <p className='font-[PretendardVariable] text-[16px] text-[#BDBDBD]'>나의 포인트</p>
                <ChevronRight size={20} color='#BDBDBD' />
              </div>
              <p className='font-[PretendardVariable] text-[18px] font-semibold'>{profile.totalPoints.toLocaleString()}P</p>
            </div>
            <div className='bg-[#FFFFFF] flex-1 flex flex-col justify-center rounded-xl p-5 shadow-[0_0_10px_0_rgba(0,0,0,0.08)] mx-auto cursor-pointer' onClick={() => navigate('/ranking')}>
              <div className='flex items-center'>
                <p className='font-[PretendardVariable] text-[16px] text-[#BDBDBD]'>나의 순위</p>
                <ChevronRight size={20} color='#BDBDBD' />
              </div>
              <p className='font-[PretendardVariable] text-[18px] font-semibold'>{ranking?.ranking}위</p>
            </div>
          </div>
        </div>
      }
